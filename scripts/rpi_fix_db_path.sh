...
def get_counts(db_path: Path) -> tuple[int, int]:
...
for p in home.rglob("results.db"):
...
if not rows:
    print("No candidate results.db files found.")
    raise SystemExit(0)

rows.sort(key=lambda x: (x[1], x[2], str(x[0])))
best_path, best_results, best_vals = rows[-1]

target_results, target_vals = get_counts(target)

print("Detected database candidates:")
for p, r, v in rows:
    marker = " <- BEST" if p == best_path else ""
    marker += " <- TARGET" if p == target else ""
    print(f"- {p} | results={r} validation={v}{marker}")

if best_path == target:
    print("Target DB is already the best candidate. No migration needed.")
    raise SystemExit(0)

if (best_results, best_vals) <= (target_results, target_vals):
...
new_results, new_vals = get_counts(target)
print(f"Migrated DB to target. New counts: results={new_results} validation={new_vals}")
PY
